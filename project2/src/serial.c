...
void init_context(compression_context_t *ctx, const char *dir, char **files, int n, FILE *f_out) {
	...
	// Initialize all fields of the context structure
	ctx->directory_name = dir;
	ctx->files = files;
	ctx->file_count = n;
	
	// Initialize indices for work distribution
	ctx->next_index = 0;        // Next file to be claimed by a worker
	ctx->next_write_index = 0;   // Next file that should write to output
...
void destroy_context(compression_context_t *ctx) {
	...
	// Free the file list if it exists
	if(ctx->files != NULL) {
...
void *compression_worker(void *arg) {
	...
	while (1) {
        ...
        // write little-endian compressed length header so the reader
        // knows how many bytes belong to this file
